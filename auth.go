...
func (a *Auth) Op(key ssh.PublicKey, d time.Duration) {
	...
	fmt.Fprintf(os.Stderr, "Added to ops: %s (for %s)\n", authItem.Key(), d)
}
...
func (a *Auth) Whitelist(key ssh.PublicKey, d time.Duration) {
	...
	fmt.Fprintf(os.Stderr, "Added to whitelist: %s (for %s)\n", authItem.Key(), d)
}
...
func (a *Auth) BanFingerprint(authkey string, d time.Duration) {
	...
	fmt.Fprintf(os.Stderr, "Added to banned: %s (for %s)\n", authItem.Key(), d)
}
...
func (a *Auth) BanAddr(addr net.Addr, d time.Duration) {
	...
	fmt.Fprintf(os.Stderr, "Added to bannedAddr: %s (for %s)", authItem.Key(), d)
}
